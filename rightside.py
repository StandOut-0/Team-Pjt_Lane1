...
def render_right_sidebar(col):
    ...
    with col:
        if st.session_state.right_sidebar_open:
            # Search toggle block
            if st.button("🔍 충전소 검색 열기" if not st.session_state.search_section_open else "🔽 충전소 검색 닫기", key="search_toggle"):
                st.session_state.search_section_open = not st.session_state.search_section_open
                st.rerun()
